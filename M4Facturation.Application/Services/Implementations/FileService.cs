...
public class FileService : BaseService, IFileService
{
    ...
    public async Task<OperationResponse<File>> Post(File file)
    {
        try
        {
            ...
            var s3Client = new AmazonS3Client(
                ...
                ConfigSecret,
                endpoint);
            var nameFile = ObtenerNombreYExtension(file.NameFile);
            var bytes = Convert.FromBase64String(file.Base64);
            var archivoS3 = file.Ruta + nameFile;
            using (s3Client)
            {
                ...
                using (var ms = new MemoryStream(bytes))
                {
                    ...
                    return response.HttpStatusCode == System.Net.HttpStatusCode.OK
                        ? Ok(file)
                        : ServerErrorFile<File>(string.Empty);
                }
            }
        }
        catch (Exception ex)
        {
            return ServerErrorFile<File>(ex.Message);
        }
    }
